package com.physwf.components.map.wayfinding.astar
{
	...
	public class QuickSortAstar implements IAstar
	{
		...
		public function QuickSortAstar()
		{
			...
			while(minCostNode != _endNode)
			{
				...
				for(var dx:int=-1;dx<=1;++dx)
				{
					...
					for(var dy:int=-1;dy<=1;++dy)
					{
						...
						if((_openList.indexOf(neerNode)>-1) || (_closedList.indexOf(neerNode)>-1))
						{
							if(neerNode.f > f)
							{
								neerNode.f = f;
				...
				if(!_openList.length) return false;//no path found
				_openList.sort(function (A:INumerical,B:INumerical):int
				{
					if(A.value > B.value)
					{
						return 1;
				...
				minCostNode = _openList.shift() as Node;
			}//end while
			buildPath();
			return true;
		}
