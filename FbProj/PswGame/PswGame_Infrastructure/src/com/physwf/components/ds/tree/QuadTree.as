package com.physwf.components.ds.tree
{
	...
	public class QuadTree
	{
		...
		/**
		 * 添加可视对象到树中 
		 * @param obj 类型为IMeasurables
		 */		
		public function insertObj( obj:IMeasurable ):void{
			var sonNode:Node = searchNodeByPoint(new Point(obj.rect.x,obj.rect.y) , _mainNode );
		 ...
		 * @param obj
		 */		
		public function deleteObj (obj:IMeasurable):void{
			var sonNode:Node = searchNodeByPoint(new Point(obj.rect.x,obj.rect.y) , _mainNode );
		 ...
		 * @return 该区域的显示对象集合
		 */		
		public function searchByRect( rect:Rectangle , exact:Boolean ):Vector.<IMeasurable>{
			var objVec:Vector.<IMeasurable> = new Vector.<IMeasurable>();
			if(_mainNode!=null){ 
		 ...
		 * @param tempNode
		 */		
		private function queryAndAdd(objVec:Vector.<IMeasurable>,rect:Rectangle ,  tempNode:Node ,exact:Boolean ):void{
			//如果没有交集，则返回
			...
			if(tempNode.hasSon){
				...
				for each(var obj:IMeasurable in tempNode.objVec){
					if(exact){
						var sonRect:Rectangle = obj.rect ;
...
class Node{
	...
	//此节点下所有的对象集合
	public var objVec:Vector.<IMeasurable> = new Vector.<IMeasurable>();
	//此节点的儿子节点集合
	...
	public function dispose():void
	{
		...
		if(objVec)
		{
			for each( var mc:IMeasurable in objVec)
			{
				mc = null ;
			}
			objVec = new Vector.<IMeasurable>(); ;
		}
	}
}